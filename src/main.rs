...
use tokio_tungstenite::{accept_async, connect_async, tungstenite::Error, WebSocketStream, tungstenite::Message, MaybeTlsStream};
...
async fn handle_message(message: String, peer_connection: Arc<Mutex<RTCPeerConnection>>) -> Result<()> {
    ...
    match signal_message {
        SignalResponse::IceCandidate { ice_candidate } => {
            ...
            let mut lock = peer_connection.lock().await;
    ...
    while let Some(message) = incoming.next().await {
        match message {
            Ok(Message::Text(text)) => {
                ...
                handle_message(text, Arc::clone(&peer_connection)).await;
            },
...
async fn main() -> Result<()> {    
    ...
    let (signal_outgoing, mut signal_incoming) = ws_stream.split();
    ...
    lock.on_ice_candidate(Box::new(move |candidate: Option<RTCIceCandidate>| {
        ...
        Box::pin(async move {
            if let Some(candidate) = candidate {
                ...
                ws_outgoing.send(Message::Text(ice_request_json)).await;
            }
        })
    }));
